...
| `nbdi-deadline-alarm-keeps-its-default-action` | a Python `SIGALRM` handler, which runs only once the interpreter regains control and so misses the deadline of a blocked resolver |
