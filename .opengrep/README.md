...
| id | What it refuses |
| --- | --- |
| `nbdi-tests-use-public-coordinator` | a private attribute read on `ImportEngine` or on any value bound from it |
| `nbdi-tests-use-public-coordinator-direct` | the same read taken straight off the class, a fresh instance, the module, or `getattr` |

Taint mode carries the first rule through any binding form, so an alias, a tuple target, a walrus,
an attribute target, and an `as` import are all covered without enumerating them. It is
