...
Two shapes are flagged, because they produce the same object:
  ...
  * **Patching our own code** — ``patch("netbox_data_import.x.y")`` or ``patch.object(Server, "z")``
    ...
    Only ``netbox_data_import`` targets count: patching a third-party or stdlib target
    (``django.utils.timezone.now``, an HTTP client) is the endorsed way to stub a boundary.
...
carve out the legitimate cases three ways —
  ...
  1. **Bound it.** ``MagicMock(spec=ImportProfile)`` / ``spec_set=`` / ``create_autospec`` /
  ...
  3. **Grandfather it.** ``netbox_data_import/tests/mock_discipline_baseline.txt`` records how many
     ...
     intentional change with::
         ...
         python3 netbox_data_import/tests/mock_discipline.py --update-baseline
...
Names are resolved through the lexical scopes rather than matched textually, and that is what makes
the guard usable here: this suite is full of ``api.patch(...)`` and ``self.client.patch(...)`` REST
calls, and a name match would flag every one of them. Do not "simplify" ``_canonical_binding`` away.

Stdlib-only by design: this module imports nothing from ``netbox_data_import`` (which would pull in
NetBox/Django), so it runs as a standalone pre-commit hook on the host without a NetBox
install. Run it directly (``python3 netbox_data_import/tests/mock_discipline.py``); the pytest suite
also imports it as ``netbox_data_import.tests.mock_discipline`` (see ``test_mock_discipline.py``).
"""
...
class _Scanner(ast.NodeVisitor):
    ...
    def _unspecced_first_party_patch(self, node: ast.Call) -> str | None:
        ...
        ``patch("netbox_data_import.x.y")`` hands the caller a plain ``MagicMock``: it accepts any
...
def save_baseline(counts: dict[str, int], path: Path = _BASELINE_PATH) -> None:
    ...
    # REUSE-IgnoreStart — the literals below are the generated header, not this file's own tags.
    header = [
        "# SPDX-FileCopyrightText: 2026 Marcin Zieba <marcinpsk@gmail.com>",
        "# SPDX-License-Identifier: Apache-2.0",
        "# Mock-discipline baseline — grandfathered spec-less MagicMock/Mock usages.",
        "# Each line: <relpath-from-netbox_data_import/tests>::<qualname>\\t<allowed-count>.",
        ...
        "# change with:  python3 netbox_data_import/tests/mock_discipline.py --update-baseline",
        "",
    ]
    # REUSE-IgnoreEnd
    body = [f"{site}\t{counts[site]}" for site in sorted(counts)]
    # rstrip: an empty body would otherwise leave a blank line the end-of-file-fixer strips.
