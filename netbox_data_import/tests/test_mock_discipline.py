...
See ``netbox_data_import/tests/mock_discipline.py`` for the policy: spec-less MagicMock/Mock used as
...
from netbox_data_import.tests import mock_discipline as md
from netbox_data_import.tests.mock_discipline import (
    Violation,
    _comment_lines,
    _counts_by_site,
    load_baseline,
...
def test_no_unapproved_mocks_beyond_baseline():
    ...
    resort regenerate the baseline: ``python3 netbox_data_import/tests/mock_discipline.py --update-baseline``.
    """
    bad = unapproved()
    assert not bad, (
        ...
        + (
            ...
            "`# mock-ok: <reason>`. Last resort: python3 netbox_data_import/tests/mock_discipline.py --update-baseline"
        )
    )
...
def test_flags_keyword_form_policy_patches():
    src = (
        "from unittest.mock import patch\n"
        "from netbox_data_import import inference_backend\n"
        "patch.object(target=inference_backend, attribute='proposal_eligible_set_limit', new=lambda: 1)\n"
        "patch(target='netbox_data_import.inference_backend.proposal_eligible_set_limit', new=lambda: 1)\n"
    )
...
def test_flags_keyword_form_first_party_patch_targets():
    src = (
        ...
        "from netbox_data_import import views\n"
        "patch.object(target=views, attribute='helper')\n"
        "patch(target='netbox_data_import.views.helper')\n"
        "patch.multiple(target='netbox_data_import.views', helper=DEFAULT)\n"
    )
    assert [hit.kind for hit in scan_source(src)] == ["patch", "patch", "patch"]
...
def test_accepts_inline_marker():
    src = (
        "from unittest.mock import MagicMock\n\n"
        "def test_x():\n"
        "    client = MagicMock()  # mock-ok: external HTTP boundary\n"
        "    return client\n"
    )
    assert scan_source(src, "t.py") == []
...
def test_flags_unspecced_patch_of_first_party_target():
    """``patch("netbox_data_import...")`` yields the same fabricating MagicMock as ``MagicMock()``."""
    src = _PATCH_IMPORT + '\n@patch("netbox_data_import.views.leases.fetch_subnet_choices")\ndef test_x(m):\n    pass\n'
    hits = scan_source(src, "t.py")
    assert len(hits) == 1
    assert hits[0].kind == "patch"
    assert hits[0].qualname == "test_x"
...
def test_accepts_autospecced_patch():
    src = (
        _PATCH_IMPORT
        + '\n@patch("netbox_data_import.views.leases.fetch_subnet_choices", autospec=True)\ndef test_x(m):\n    pass\n'
    )
    assert scan_source(src, "t.py") == []
...
def test_accepts_patch_given_a_ready_made_replacement():
    ...
    for call in (
        'patch("netbox_data_import.x.y", new=object())',
        'patch("netbox_data_import.x.y", new_callable=lambda: object())',
        'patch("netbox_data_import.x.y", object())',
    ):
...
def test_flags_patch_with_unbounded_mock_new_callable():
    ...
    for mock_class in ("MagicMock", "NonCallableMagicMock", "Mock", "NonCallableMock", "AsyncMock"):
        src = (
            ...
            "def test_x():\n"
            f'    with patch("netbox_data_import.x.y", new_callable={mock_class}):\n'
            "        pass\n"
        )
...
def test_accepts_patch_with_spec_bounded_mock_new_callable():
    ...
    src = (
        ...
        "    pass\n\n"
        "def test_x():\n"
        '    with patch("netbox_data_import.x.y", new_callable=MagicMock, spec=Thing):\n'
        "        pass\n"
    )
    assert scan_source(src, "t.py") == []
...
def test_flags_patch_with_new_set_to_the_default_sentinel():
    ...
    for call in (
        'patch("netbox_data_import.x.y", new=DEFAULT)',
        'patch("netbox_data_import.x.y", new=mock.DEFAULT)',
        'patch.object(Server, "get_client", new=DEFAULT)',
    ):
        src = (
            "from unittest.mock import DEFAULT, patch\n"
            "import unittest.mock as mock\n"
            "from netbox_data_import.models import Server\n\n"
            f"def test_x():\n    with {call}:\n        pass\n"
        )
...
def test_flags_positional_default_sentinel_and_renamed_import():
    ...
    for call in (
        'patch("netbox_data_import.x.y", DEFAULT_SENTINEL)',
...
def test_flags_patch_with_none_spec():
    ...
    src = _PATCH_IMPORT + '\ndef test_x():\n    with patch("netbox_data_import.x.y", spec=None):\n        pass\n'
...
def test_flags_patch_with_false_spec_arguments():
    ...
    for argument in ("autospec", "spec", "spec_set"):
        src = (
            _PATCH_IMPORT
            + f'\ndef test_x():\n    with patch("netbox_data_import.x.y", {argument}=False):\n        pass\n'
        )
...
def test_accepts_patch_with_falsy_non_boolean_spec_arguments():
    ...
    for argument in ("autospec", "spec", "spec_set"):
        for value in ("0", '""'):
            src = (
                _PATCH_IMPORT
                + f'\ndef test_x():\n    with patch("netbox_data_import.x.y", {argument}={value}):\n        pass\n'
            )
...
def test_accepts_unrelated_values_named_default_as_replacements():
    ...
    for replacement in ("DEFAULT", "settings.DEFAULT"):
        src = (
            ...
            "class settings:\n"
            "    DEFAULT = object()\n\n"
            f'def test_x():\n    with patch("netbox_data_import.x.y", new={replacement}):\n        pass\n'
        )
...
def test_accepts_imported_default_shadowed_at_module_scope():
    ...
    src = (
        "from unittest.mock import DEFAULT, patch\n\n"
        "DEFAULT = object()\n"
        'with patch("netbox_data_import.x.y", new=DEFAULT):\n'
        "    pass\n"
    )
    assert scan_source(src, "t.py") == []
...
def test_function_local_default_does_not_shadow_default_expression():
    ...
    src = (
        "from unittest.mock import DEFAULT, patch\n\n"
        'def test_x(value=patch("netbox_data_import.x.y", new=DEFAULT)):\n'
        "    DEFAULT = object()\n"
    )
    assert [hit.kind for hit in scan_source(src, "t.py")] == ["patch"]
...
def test_flags_default_through_unittest_package_alias():
    ...
    src = (
        ...
        "from unittest.mock import patch\n\n"
        'def test_x():\n    with patch("netbox_data_import.x.y", new=ut.mock.DEFAULT):\n        pass\n'
    )
    assert [hit.kind for hit in scan_source(src, "t.py")] == ["patch"]
...
def test_ignores_unrelated_attribute_patch_calls():
    ...
    src = (
        ...
        "from netbox_data_import.models import Server\n\n"
        "def test_x():\n"
        '    tool.patch("netbox_data_import.x.y")\n'
...
def test_flags_patch_through_mock_module_aliases():
    ...
    for import_statement, patch_name in cases:
        src = import_statement + f'\ndef test_x():\n    with {patch_name}("netbox_data_import.x.y"):\n        pass\n'
...
def test_ignores_mock_module_alias_rebound_in_function():
    ...
    src = (
        ...
        "def test_x():\n"
        "    mock = tool\n"
        '    mock.patch("netbox_data_import.x.y")\n'
    )
    assert scan_source(src, "t.py") == []
...
def test_ignores_patch_dict():
    ...
    src = "from unittest.mock import patch\nfrom netbox_data_import import constants\n\ndef test_x():\n    with patch.dict(constants.THING, {}):\n        pass\n"
    assert scan_source(src, "t.py") == []
...
def test_patch_marker_opt_out_is_honoured():
    src = (
        ...
        + '@patch("netbox_data_import.jobs.ImportJobRunner")\ndef test_x(m):\n    pass\n'
    )
    assert scan_source(src, "t.py") == []
...
def test_patch_and_mock_violations_share_one_baseline_budget(tmp_path):
    ...
    src = (
        ...
        "def test_x():\n"
        "    row = MagicMock()\n"
        '    with patch("netbox_data_import.x.y"):\n'
        "        pass\n"
    )
    hits = scan_source(src, "t.py")
...
def test_patch_violation_str_names_the_target():
    v = Violation("sub/test_x.py", 12, "TestC.test_y", "'netbox_data_import.x.y'", kind="patch")
    assert str(v) == "sub/test_x.py:12: unspecced patch of first-party 'netbox_data_import.x.y' in TestC.test_y()"
...
def test_accepts_a_nested_partial_bound_at_the_inner_layer():
    """The inner binding survives because no outer layer overrides it."""
    src = (
        "import functools\nfrom unittest.mock import MagicMock, patch\nclass C: ...\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', "
...
def test_flags_an_aliased_partial_of_a_specless_mock():
    src = (
        "from functools import partial as make_partial\n"
        "from unittest.mock import MagicMock, patch\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', new_callable=make_partial(MagicMock)):\n"
        "        pass\n"
    )
    assert [h.mock for h in scan_source(src, "t.py")] == ["'netbox_data_import.views.thing'"]


def test_accepts_a_local_partial_that_returns_a_real_object():
    src = (
        "from unittest.mock import MagicMock, patch\n\n"
        "def partial(factory):\n"
        "    return object()\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', new_callable=partial(MagicMock)):\n"
        "        pass\n"
    )
    assert scan_source(src, "t.py") == []


def test_flags_a_partial_of_a_specless_mock_through_a_module_alias():
    src = (
        "import functools as ft\nfrom unittest.mock import MagicMock, patch\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', new_callable=ft.partial(MagicMock)):\n"
        "        pass\n"
    )
    assert [h.mock for h in scan_source(src, "t.py")] == ["'netbox_data_import.views.thing'"]


def test_flags_a_nested_partial_with_an_outer_none_spec_override():
    src = (
        "import functools\nfrom unittest.mock import MagicMock, patch\nclass C: ...\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', "
        "new_callable=functools.partial(functools.partial(MagicMock, spec=C), spec=None)):\n"
        "        pass\n"
    )
    assert [h.mock for h in scan_source(src, "t.py")] == ["'netbox_data_import.views.thing'"]


def test_accepts_a_nested_partial_bound_at_the_outer_layer():
    src = (
        "import functools\nfrom unittest.mock import MagicMock, patch\nclass C: ...\n\n"
        "def test_x():\n"
        "    with patch('netbox_data_import.views.thing', "
