...
The sync also migrates `data_import_source.extra.primary_contact` from existing Devices. It removes only `primary_contact` from the JSON data and preserves other extra values. A failed contact sync rolls back the Device update and keeps the legacy JSON value.
